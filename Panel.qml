...
// Omarchy's Sound panel, plus AirPods battery and listening modes.
//
// Panel.qml and Model.js are vendored from Omarchy 4.0's first-party
// omarchy.audio plugin (MIT, see LICENSE) because the shell has no API for one
// plugin to add a section to another plugin's panel -- the supported route is to
// take a copy. Everything AirPods-specific is marked "AirPods mod" so the
// vendored parts stay easy to diff against upstream on an Omarchy update.
Panel {
  id: root
  moduleName: "community.sound-airpods-mod"
  ipcTarget: "community.sound-airpods-mod"

  // ---- AirPods mod ----------------------------------------------------------
  // State comes from the plugin's `service` entry point, which the shell loads
  // once, rather than from a per-screen Process. See Service.qml.
  readonly property var pods: bar?.shell?.serviceFor("community.sound-airpods-mod") ?? null
  readonly property bool podsConnected: pods ? pods.connected : false

  // Only speak for the AirPods when they're the selected output, the way macOS
  // only offers Noise Control for the device you're listening through.
  readonly property bool podsIsActiveOutput: podsConnected && isPodsNode(root.sink)

  // Show the battery reading next to the speaker glyph in the bar. Off by
  // default so the bar looks exactly like the stock Sound widget:
  ...
  function isPodsNode(node) {
    ...
    return String(node.name || "").toLowerCase().indexOf(needle) !== -1
  }

  readonly property var ancCatalogue: pods ? [
    { value: pods.ancOff, label: "Off", icon: "󰂲" },
    { value: pods.ancTransparency, label: "Transparency", icon: "󰋋" },
    { value: pods.ancNoiseCancellation, label: "Noise Cancellation", icon: "󰓃" }
  ] : []

  // Only the modes this particular device advertises.
  readonly property var ancModes: {
    if (!podsIsActiveOutput || !pods) return []
    var options = pods.ancOptions || 0
    var out = []
    for (var i = 0; i < ancCatalogue.length; i++)
      if (options & ancCatalogue[i].value) out.push(ancCatalogue[i])
    return out
  }
  ...
  function podsBatterySummary(node) {
    if (!isPodsNode(node)) return ""
    var parts = []
    var components = pods ? pods.batteryComponents : []
    ...
    PanelKeyCatcher {
      ...
      ScrollView {
        ...
        Column {
          ...
          // ---- AirPods mod: Noise Control ----
          // Both the separator and the section disappear when no AirPods are
          // connected, so the panel is then identical to the stock one.
          PanelSeparator {
  ...
  // AirPods mod: one listening mode, styled like SinkRow so the section reads as
  // part of the panel. Mouse-driven only -- it deliberately stays out of the
  // panel's keyboard cursor model (focusSection/selectedIndex), which is indexed
  // against the audio device lists.
  component AncRow: CursorSurface {
    id: ancRow
    required property int mode
    required property string label
    required property string icon

    readonly property bool isActive: root.pods ? root.pods.effectiveAnc === mode : false
    ...
    implicitHeight: ancInner.implicitHeight + Style.spacing.xl
    ...
    Row {
      ...
      Text {
        ...
        font.bold: ancRow.isActive
        elide: Text.ElideRight
        width: parent.width - Style.space(22) - Style.space(8)
               - (ancCheck.visible ? ancCheck.implicitWidth + Style.space(8) : 0)
        anchors.verticalCenter: parent.verticalCenter
      }

      // The tick marks the live mode, the way macOS's Noise Control does.
      Text {
        id: ancCheck
        text: "󰄬"
        visible: ancRow.isActive
        color: root.bar.foreground
        font.family: root.bar.fontFamily
        font.pixelSize: Style.font.body
        anchors.verticalCenter: parent.verticalCenter
    ...
    MouseArea {
      ...
      onClicked: if (root.pods) root.pods.setAnc(ancRow.mode)
    }
  }
